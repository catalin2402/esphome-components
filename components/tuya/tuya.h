...
enum class TuyaFeatureConfigurationSubcommand : uint8_t {
  NOTIFY = 0x00,
};
...
enum class TuyaNetworkStatus : uint8_t {
  EZ_PAIRING = 0x00,
  AP_PAIRING = 0x01,
  CONFIGURED_NOT_CONNECTED = 0x02,
  ROUTER_CONNECTED = 0x03,
  CLOUD_CONNECTED = 0x04,
  LOW_POWER = 0x05,
  EZ_AND_AP_PAIRING = 0x06,
};

enum class TuyaNetworkMode : uint8_t {
  UNKNOWN = 0x00,
  COORDINATED,
  SELF_PROCESSING,
};
...
class Tuya : public Component, public uart::UARTDevice {
 ...
 protected:
  ...
  void handle_feature_configuration_(const uint8_t *buffer, size_t len);
  void update_protocol_version_(uint8_t version);
  void start_initialization_();
  void begin_datapoint_sync_();
  void complete_initialization_();
  void send_raw_command_(TuyaCommand command);
  void send_response_(TuyaCommandType command, const std::vector<uint8_t> &payload);
  ...
  TuyaNetworkStatus get_wifi_status_();
  uint8_t get_wifi_rssi_();

#ifdef USE_TIME
  void send_local_time_(bool immediate = false);
  time::RealTimeClock *time_id_{nullptr};
  bool time_sync_callback_registered_{false};
#endif

  TuyaInitState init_state_{TuyaInitState::INIT_HEARTBEAT};
  TuyaNetworkMode network_mode_{TuyaNetworkMode::UNKNOWN};
  optional<uint8_t> protocol_version_{};
  optional<TuyaNetworkStatus> wifi_status_{};
  bool wifi_status_reporting_enabled_{false};
  ...
  int bluetooth_status_pin_reported_{-1};
